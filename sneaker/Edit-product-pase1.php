...
<body>  
    ...
    <div class="register-maket">
        <div class="container">
            <form action="PHP-Edit-Market.php" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()" required>
              <div class="Greid-register-market">
                <?php
                   if (!empty($_SESSION['member_id'])) {
                      $member_id = $_SESSION['member_id'];
                      $res_file_market = $conn->query("SELECT * From market_info Where mrk_id = '$member_id'");
                  }
                ?>   
                <?php while($row = $res_file_market->fetch_array()):?> 
                <div class="Gride-right-register-market">
                    ...
                    <label class="tele-add-img-market">อัพโหลดโปรไพล์</label>
                        <a href="#"><img src='uploadprofile/<?= $row["mrk_pic"]?>' alt="Seller-img" class="add-img-market"></a>
                    ...
                    <div class="add-contact-market">
                        <input class="add-contact-market-facebook" type="text" name="mrk_fb" value="<?php echo $row["mrk_fb"];?>" placeholder="Facebook" required maxlength="25">
                        <input class="add-contact-market-ig" type="text"  name="mrk_ig" value="<?php echo $row["mrk_ig"];?>" placeholder="Instagram" required maxlength="25">
                        <input class="add-contact-market-LineID" type="text"  name="mrk_line" value="<?php echo $row["mrk_line"];?>" placeholder="LineID" required maxlength="25">
                    </div>  
                    <label class="tele-add-timeOpen-market">เวลาเปิดทำการหรือเวลาที่สามารถติดต่อ</label>
                    <div class="add-timeOpen-market">
                        
                        <input type="time" id="appt" name="mrk_open" value="<?php echo $row["mrk_open"];?>"class="drowdown-timeOpen" required> 
                        
                        <input type="time" id="appt" name="mrk_close" value="<?php echo $row["mrk_close"];?>" class="drowdown-timeClose" required>
                    </div>    
                    ...
                    <button class="cancle-regiter-merket" name="Exit" value="Exit">ยกเลิก</button>
                </div>
                <div class="Gride-left-register-market">
                    ...
                    <?php endif ?>
                    <input class="textbox-add-name-market" type="text" name="mrk_name" value="<?php echo $row["mrk_name"];?>" required minlength="5" maxlength="30">
                    <input class="textbox-add-name-market" type="text" name="mrk_name_befor" value="<?php echo $row["mrk_name"];?>"  style="display: none;">
                    <label class="tele-add-IDcade-market">รหัสบัตรประชาชน</label>
                    <input class="textbox-add-IDcade-market" type="number" name="mrk_personal_id" value="<?php echo $row["mrk_personal_id"];?>" required  pattern="\d*" step="0.01" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength = "13" />
                    <label class="tele-add-contact-phone-market">เบอร์ติดต่อ</label>
                    <input class="textbox-add-contact-phone-market" type="number" name="mrk_phone" value="<?php echo $row["mrk_phone"];?>" required pattern="\d*" step="0.01" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength = "10" />
                    ...
                    <div class="add-addaress-market" >
                        <div class="gried-addaress-maket">
                            <input class="textbox-add-addaress-market" type="text" name="mrk_address" value="<?php echo $row["mrk_address"];?>" placeholder="ที่อยู่" required maxlength="30">
                            <input class="textbox-add-canton-market" type="text" name="mrk_sub_district" value="<?php echo $row["mrk_sub_district"];?>" placeholder="ตำบล/แขวง" required maxlength="30">
                            <input class="textbox-add-district-market" type="text" name="mrk_district" value="<?php echo $row["mrk_district"];?>" placeholder="อำเภอ/เขต" required  maxlength="30">
                            <input class="drowdown-add-city" name="mrk_city" id="mrk_city" value="<?php echo $row["mrk_city"];?>"  placeholder="จังหวัด" required maxlength="30">
                            <input class="textbox-add-Postal-code-market" type="text" value="<?php echo $row["mrk_zipcode"];?>" name="mrk_zipcode" placeholder="รหัสไปรษณีย์" required  maxlength="30">
                        </div>

                    </div>
                    <label class="tele-add-Latitude-longitude-market">ตำแหน่งละติจูดและลองจิจูด</label>
                    <div class="gried-Latitude-longitude-maket">
                    <input class="textbox-add-Latitude-market" type="text" name="mrk_lat_location" value="<?php echo $row["mrk_lat_location"];?>" required minleght="10" maxlength="25"> 
                    <input class="textbox-add-longitude-market" type="text" name="mrk_lgt_location" value="<?php echo $row["mrk_lgt_location"];?>" required minleght="10" maxlength="25">         <div class="Latitude-longitude-market">
